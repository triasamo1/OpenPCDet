...
#@numba.jit(nopython=True)
def compute_statistics_jit_trias(overlaps,
                           gt_datas,
                           dt_datas,
                           ignored_gt,
                           ignored_det,
    ...
    # trias - get center location
    dt_3D_bboxes = dt_datas[:, 5:8]
    gt_3D_bboxes = gt_datas[:, 5:8]

    # trias - create an empty list of lists to append the True Positive bbox distance data ####################################################################
    list_center_data = []
    list_center_data_raw = []
    # temp_list=[]
    ...
    if compute_fp:
        ...
        if compute_aos:
            ...
            if tp > 0 or fp > 0:
                ...
                similarity = -1
    return tp, fp, fn, similarity, thresholds[:thresh_idx], list_center_data ,list_center_data_raw ,temp_list
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    rets = compute_statistics_jit(
                        ...
                        dontcares[i],
                        metric,
                        min_overlap=min_overlap,  # hard coded: min_overlap
                        thresh=0.0,                       
                        compute_fp=False)
...
def eval_class_trias(gt_annos,
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                thresholdss = []
                for i in range(len(gt_annos)):
                    rets = compute_statistics_jit_trias(
                        overlaps[i],
                        gt_datas_list[i],
                        dt_datas_list[i],
                        ignored_gts[i],
                        ...
                        thresh=0.0,                       
                        compute_fp=False)
                    tp, fp, fn, similarity, thresholds, list_of_center_data, list_of_center_data_raw, temp_list_forstats = rets # trias ################################
                    list_gather_all = list_gather_all + list_of_center_data #trias
                    if (list_of_center_data_raw not in temp_list):
                        temp_list.append(list_of_center_data_raw)
                        list_gather_all_raw = list_gather_all_raw + list_of_center_data_raw #trias
    ...
    with open('/home/triasamo/pointpillars_cls2_xyz_PEDCYCL.csv', 'w', newline='') as file:  # trias write to csv
...
def do_eval(gt_annos,
    ...
    ret = eval_class_trias(gt_annos, dt_annos, current_classes, difficultys, 1,
                     min_overlaps)
